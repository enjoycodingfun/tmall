...
<div>
    ...
    <form id="form-search" class="page-container">
        ...
        <div class="cl pd-5 bg-1 bk-gray mt-20"> <span class="l"><a href="javascript:;" onclick="datadel()" class="btn btn-danger radius"><i class="Hui-iconfont">&#xe6e2;</i> 批量删除</a> <a class="btn btn-primary radius" onclick="product_add('添加商品','product-add')" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加商品</a></span>
            <span class="r">
                <input type="text" name="searchKey" id="searchKey" placeholder=" 商品名称、商品描述、价格" style="width:250px" class="input-text">
               <button name="" id="searchButton" type="submit" class="btn btn-success"><i class="Hui-iconfont">&#xe665;</i> 搜商品</button>
            </span> </div>
...
<script type="text/javascript">
            ...
            "columns": [
                { "data": "id",
                    render : function(data,type, row, meta) {
                        return "<input name=\"checkbox\" value=\""+row.pid+"\" type=\"checkbox\" value=\"\">";
                    }
                },
                { "data": "pid"},
                { "data": "pname",
                ...
                },
                {
                    ...
                    render: function (data, type, row, meta) {
                        if (row.status == 1) {
                            return "<a style=\"text-decoration:none\" onClick=\"product_stop(this,"+row.pid+")\" href=\"javascript:;\" title=\"下架\"><i class=\"Hui-iconfont\">&#xe6de;</i></a> <a style=\"text-decoration:none\" class=\"ml-5\" onClick=\"product_edit('商品编辑','product-edit',"+row.pid+",'','700')\" href=\"javascript:;\" title=\"编辑\"><i class=\"Hui-iconfont\">&#xe6df;</i></a> <a style=\"text-decoration:none\" class=\"ml-5\" onClick=\"product_del(this,"+row.pid+")\" href=\"javascript:;\" title=\"删除\"><i class=\"Hui-iconfont\">&#xe6e2;</i></a>";
                        } else {
                            return "<a style=\"text-decoration:none\" onClick=\"product_start(this,"+row.pid+")\" href=\"javascript:;\" title=\"发布\"><i class=\"Hui-iconfont\">&#xe603;</i></a> <a style=\"text-decoration:none\" class=\"ml-5\" onClick=\"product_edit('商品编辑','product-edit',"+row.pid+",'','700')\" href=\"javascript:;\" title=\"编辑\"><i class=\"Hui-iconfont\">&#xe6df;</i></a> <a style=\"text-decoration:none\" class=\"ml-5\" onClick=\"product_del(this,"+row.pid+")\" href=\"javascript:;\" title=\"删除\"><i class=\"Hui-iconfont\">&#xe6e2;</i></a>";
                        }
                    }
                }
            ],
    ...
    $("#form-search").validate({
        ...
        submitHandler:function(form){
            ...
            table.ajax.url( '${pageContext.request.contextPath}/listSearch' ).load();
        }
    });
  ...
  /* var index = layer.load(3);*/
    ...
    var setting = {
        ...
        async: {
            enable: true,
            url: "${pageContext.request.contextPath}/item/catList",
            type: "GET",
   ...
   /* $(document).ready(function(){
    ...
    /*产品-下架*/
    function product_stop(obj,pid){
        layer.confirm('确认要下架ID为\''+pid+'\'的商品吗？',{icon:0},function(index){
            var index = layer.load(3);
            $.ajax({
                type: 'GET',
                url: '${pageContext.request.contextPath}/itemStop?pid='+pid,
                dataType: 'json',
    ...
    /*产品-发布*/
    function product_start(obj,pid){
        layer.confirm('确认要发布ID为\''+pid+'\'的商品吗？',{icon:3},function(index){
            var index = layer.load(3);
            $.ajax({
                type: 'GET',
                url: '${pageContext.request.contextPath}/itemStart?pid='+pid,
                dataType: 'json',
                success: function(data){
                    layer.close(index);
                    if(data==0){
    ...
    /*产品-编辑*/
    function product_edit(title,url,pid,w,h){
        setId(pid);
        var table = $('.table').DataTable();
        $('.table tbody').on( 'click', 'tr', function () {
           /* pid = table.row(this).data().pid;*/
            pname = table.row(this).data().pname;
            price = table.row(this).data().price;
            sellPoint = table.row(this).data().sellPoint;
            cid = table.row(this).data().cid;
            status = table.row(this).data().status;
            catName = table.row(this).data().catName;
            num = table.row(this).data().num;
        });
    ...
    function product_del(obj,pid){
        layer.confirm('确认要删除ID为\''+pid+'\'的商品吗？',{icon:0},function(index){
            /*var index = layer.load(3);*/
            $.ajax({
                type: 'POST',
                url: '${pageContext.request.contextPath}/productDelete?pid='+pid,
                dataType: 'json',
                success: function(data){
                    layer.close(index);
                    if(data==0){
    ...
    function datadel() {
        ...
        layer.confirm('确认要删除所选的'+count+'条数据吗？',{icon:0},function(index){
           /* var index = layer.load(3);*/
            $.ajax({
                type: 'GET',
                url: '${pageContext.request.contextPath}/removeProduct?ids='+ids,
                dataType: 'json',
                success:function(data){
                    layer.close(index);
                    if(data==0){
