...
//    datatables配置
            ...
            "columns": [
               ...
               /* { "data": "ordertime",
                ...
                }*/
                {
                    "data": null,
                    render : function(data, type, row, meta) {
                        return "   <a title=\"查看详情\" href=\"${pageContext.request.contextPath}/orderItem-list?oid="+data.oid+"\" class=\"ml-5\" style=\"text-decoration:none\"><i class=\"Hui-iconfont\">&#xe634;</i></a> " +
                                "   <a title=\"编辑\" href=\"javascript:;\" onclick=\"order_edit('编辑','order-edit','"+data.oid+"','','510')\" class=\"ml-5\" style=\"text-decoration:none\"><i class=\"Hui-iconfont\">&#xe6df;</i></a>" +
                                "   <a title=\"删除\" href=\"javascript:;\" onclick=\"order_del('"+data.oid+"')\" class=\"ml-5\" style=\"text-decoration:none\"><i class=\"Hui-iconfont\">&#xe6e2;</i></a> ";
                    }
                }
            ],
    ...
    function orderCount() {
        $.ajax({
            url:"${pageContext.request.contextPath}/order/getOrderConut",
            type: 'GET',
    ...
    function order_del(oid){
        layer.confirm('确认要删除ID为\''+oid+'\'的订单吗？',function(index){
            var index = layer.load(3);
            $.ajax({
                type: 'get',
                url: '${pageContext.request.contextPath}/order/removeOrder?oid='+oid,
                dataType: 'json',
                success: function(data){
                    layer.close(index);
                    if(data.result!=1){
    ...
    function datadel() {
        ...
        layer.confirm('确认要删除所选的'+count+'条数据吗？',{icon:0},function(index){
            ...
            $.ajax({
                type: 'get',
                url: '${pageContext.request.contextPath}/order/removeListOrder?ids='+ids,
                dataType: 'json',
                success:function(data){
                    layer.close(index);
                    if(data.result!=1){
